...
Predicting customer demand is essential for a growing online business. Without accurate forecasts, we risk running out of stock or having too much inventory, which can lead to wasted resources. For a food delivery service like ours, this is even more important because we deal with fresh ingredients that don’t last long. By accurately estimating daily and weekly demand, we can ensure we have just the right amount of stock—minimizing waste, keeping costs under control, and ensuring our customers always get what they want.
...
train <- train |>
  mutate(profit = ifelse(promotions == "No", 
    ...
    emailer_for_promotion == 0 & homepage_featured == 1 ~ "Web promotion",
...
We examined data from `{r} length(unique(train$center_id))` stores over a period of `{r} length(unique(unique(train$week)))` weeks.  
During this time, we tested two types of promotions (web, email) :\
...
Our goals were:\
To see if promotions boosted our bottom line\
...
tst|>
  ...
  scale_color_manual(values = c("Double promotion" = "navy", "Email promotion" =
                                  "lightblue", "Web promotion" = "#167bb2","No promotion" = "darkred"),
                     name = "Promotion") +
...
*Sales Volume*\
Figure 2A shows how many orders were placed under different types of promotions.The y-axis is in a log scale (log2), meaning the values grow exponentially. The median order volume is highest for double promotions (both email and web), followed by web promotions, no promotions, and email promotions. The spread (height of the box) indicates variability, with double promotions showing more fluctuation in sales. The presence of numerous outliers at the upper end indicates that some stores experience exceptionally high sales under promotional influence.
...
*Profit*\
Revenue is also log-transformed for better visualization as in figure 2C. Similar to the sales volume trend, double promotions generate the highest median revenue, followed by web promotions, no promotions, and email promotions. The spread of revenue is wider, meaning revenue varies significantly under all conditions. The presence of high-value outliers suggests that under certain conditions, promotions can lead to exceptionally high revenue generation.

*Key Takeaways*\
While double promotions drive the highest sales and revenue, web and email promotions individually are not as effective as using both together. Checkout price per transaction remains stable regardless of the promotion type, meaning the increase in revenue comes from selling more, not from charging more.
...
**Imapact of Promotions:**\
Several meals (especially on the left side of the plot) show a significant increase in order percentage when a promotion is applied ("promotion_flag" - pink color). This indicates that promotions can be effective in boosting the popularity of certain meals.For some meals, the "Double promotion" (green color) or "Email promotion" seems to have a noticeable impact compared to "No promotion".

**Web Promotion Baseline:**\
"Web promotion" (purple color) seems to be always present at some level for all meals, which suggests that web promotion might be a baseline strategy applied to all meals. 
