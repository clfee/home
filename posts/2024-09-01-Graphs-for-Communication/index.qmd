...
**Goal :** The objective of this report is to demonstrate how to create animated maps with `ggplot2` and `gganimate` in R to track MRSA BSI incidence in the State of California.
...
We will generate an animated map displays whether MRSA BSI incidence in the State of California was the same (no different), better (lower) or worse (higher) than the national baseline from 2019 – 2023. The comparison is depicted values and colors range from -1 (purple, worse than the national average) to 1 (yellow, worse than the national average).
...
As gganimate extends the grammar of graphics as implemented by ggplot2 to include the description of animation.We will need to generate a base map using ggplot2. Then, add a function of facet_wrap(Hospital_Category_RiskAdjustment) to track the average performance of each hospital category across counties in California. 

### Step 2
Finally, the gganimate library is used to animate the display of points by year (2019 – 2023). Here we define the transition time (in years), add a title and subtitle, animate the map, and save it to a gif.

Note: transition_*() defines how the data should be spread out and how it relates to itself across time.
...
a + transition_time(Year) +labs(title = "MRSA BSI incidence compares to the national baseline \nsame (0), better (1) or worse (-1)   \nYear: {frame_time}")

```

### Bonus

We can apply the same strategy for other types of graph!

In this example we see a trend plot of continuous variables infections cases and days of patient hospital stays between 2019 and 2023.   
